...
//        listFiltered.forEach(System.out::println);
        ...
        List<Book> booksContainLatterEconomy = bookList.stream().filter(i -> i.getBookName().contains("경제")).collect(Collectors.toList());
//        booksContainLatterEconomy.forEach(System.out::println);

//        bookList.stream().mapToDouble(v->v.getPrice());

        double maxPrice = bookList.stream().mapToDouble(v->v.getPrice()).max().orElseThrow(NoSuchElementException::new);
//        double maxPrice2 = bookList.stream().sorted(Comparator.comparingDouble(i.getPrice))
//        System.out.println("책 목록 중 가장 비싼 금액: " + maxPrice);
//        System.out.println();

        // 카테고리가 IT인 책들의 가격 합 조회
        double sum = bookList.stream().filter(i->i.getCategory().equals("IT")).collect(Collectors.summingDouble(Book::getPrice));
        sum = bookList.stream().filter(i->i.getCategory().equals("IT")).mapToDouble(v->v.getPrice()).sum();
//        System.out.println("카테고리 IT 책들의 가격 합: " + sum);
//        System.out.println();

        // IT 책 할인 이벤트!
        List<Book> discountedBookList = bookList.stream().filter(i->i.getCategory().equals("IT"))
                .map(b-> {
                    double salePrice = b.getPrice() * 0.6;
                    b.setPrice(salePrice);
                    return b;
